...
object DuplicateRecords  extends  App {
  ...
  val countsRDD: RDD[(String, Int)] = acctDF.rdd.map(rec => (rec(0), 1)).reduceByKey(_+_).map(rec=> (rec._1.toString, rec._2))
 ...
 val countsDF: DataFrame = sparksession.createDataFrame(countRowRDD)

  //val countsDF1: DataFrame = acctDF.map(rec => (rec(0), 1)).groupBy($"AcctId").sum("value")
 // val accJoinedDF: DataFrame = acctDF.join(countsDF, acctDF("AcctId")===countsDF("AcctId"), "left_outer").select(acctDF("AcctId"), acctDF("Details"), countsDF("AcctCount"))
  acctDF show false
  countsDF show  false
